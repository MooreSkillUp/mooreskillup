from django.urls import path

from .views import (
    AdminRegisterView,
    AdminUserLockView,
    AdminUserLogoutAllView,
    AdminUserUnlockView,
    ChangePasswordView,
    CompleteOnboardingView,
    LoginView,
    LogoutAllView,
    LogoutView,
    MeView,
    MyReferralsView,
    PasswordResetConfirmView,
    PasswordResetRequestView,
    ReferralClickView,
    ReferralCodeCheckView,
    ReferralLeaderboardView,
    RefreshView,
    RegisterView,
    ResendRegisterCodeView,
    TwoFactorToggleView,
    TwoFactorVerifyView,
    UsernameAvailableView,
    VerifyRegisterView,
    health_check,
)

urlpatterns = [
    path("health/", health_check, name="health-check"),
    path("register/", RegisterView.as_view(), name="auth-register"),
    path("register/verify/", VerifyRegisterView.as_view(), name="auth-register-verify"),
    path("register/resend-code/", ResendRegisterCodeView.as_view(), name="auth-register-resend"),
    path("onboard/", CompleteOnboardingView.as_view(), name="auth-onboard"),
    path("admin-register/", AdminRegisterView.as_view(), name="auth-admin-register"),
    path("username-available/", UsernameAvailableView.as_view(), name="auth-username-available"),
    path("referrals/", MyReferralsView.as_view(), name="auth-my-referrals"),
    path("referral-code/", ReferralCodeCheckView.as_view(), name="auth-referral-code"),
    path("referral-code/click/", ReferralClickView.as_view(), name="auth-referral-click"),
    path("referrals/leaderboard/", ReferralLeaderboardView.as_view(), name="auth-referral-leaderboard"),
    path("login/", LoginView.as_view(), name="auth-login"),
    path("login/verify-2fa/", TwoFactorVerifyView.as_view(), name="auth-2fa-verify"),
    path("two-factor/", TwoFactorToggleView.as_view(), name="auth-2fa-toggle"),
    path("refresh/", RefreshView.as_view(), name="auth-refresh"),
    path("logout/", LogoutView.as_view(), name="auth-logout"),
    path("logout-all/", LogoutAllView.as_view(), name="auth-logout-all"),
    path("me/", MeView.as_view(), name="auth-me"),
    path("change-password/", ChangePasswordView.as_view(), name="auth-change-password"),
    path("password-reset/request/", PasswordResetRequestView.as_view(), name="auth-password-reset-request"),
    path("password-reset/confirm/", PasswordResetConfirmView.as_view(), name="auth-password-reset-confirm"),
    path("admin/users/<uuid:user_id>/lock/", AdminUserLockView.as_view(), name="admin-user-lock"),
    path("admin/users/<uuid:user_id>/unlock/", AdminUserUnlockView.as_view(), name="admin-user-unlock"),
    path("admin/users/<uuid:user_id>/logout-all/", AdminUserLogoutAllView.as_view(), name="admin-user-logout-all"),
]
